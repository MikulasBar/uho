...
class DbClient {
  static Future<List<(Dish, DishGlobalRating)>> fetchGlobalRatings({int limit = 10}) async {
    try {
      ...
      final ratings = result.map((json) {
        ...
        final rating = DishGlobalRating.fromJson(json['rating'] as Map<String, dynamic>);
        return (dish, rating);
      }).toList();
